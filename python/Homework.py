...
def main():
    ...
    nd_array_1 = np.array(Iris_df1)[:, :4]
    ...
    print("***Quartile Value by using Percentile***")
    ...
    # Virginica has longest pedal width and it has widest range for petal width, but Senoto is lowest
    # Violin plot is very straightforward and you can know max, min and quarile values
    # but it can only describe one character in one plot
    ...
    print("***Fit the features to a random forest***")
    random_forest = RandomForestClassifier(random_state=100)
    random_forest.fit(X, y)

    test_df = Iris_df.iloc[:, :4]
    ...
    print("***Fit the features to a Decision Tree***")
    ...
    print("***Wrap the steps into a pipeline for random forest***")
    pipeline = Pipeline(
        [
    ...
    print("***Wrap the steps into a pipeline for Decision Tree***")
    pipeline = Pipeline(
        [
            ("StandardScaler", StandardScaler()),
